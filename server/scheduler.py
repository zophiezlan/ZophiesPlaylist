...
class Scheduler(object):
    ...
    def prep_status(self, status):
        ...
        int_fields = set(
            ["cerrors", "delta", "errors", "next_run", "runs", "total", "when"]
        )
    ...
    def post_job(self, skey, when):
        ...
        # redis-py 5+ signature: zadd(name, mapping)
        pipe.zadd(self.job_queue, {skey: when})
    ...
    def get_next_item(self):
        now = self.now()
        items = self.r.zrange(self.job_queue, 0, 0, withscores=True)
        if len(items) > 0:
            item = items[0]
            skey, next_time = item
            # Cast score to int seconds if needed
            try:
                next_time = int(next_time)
            except Exception:
                next_time = int(float(next_time))
            if now >= next_time:
                self.r.zrem(self.job_queue, skey)
                return skey
        else:
    ...
    def show_info(self):
        # Count all jobs in the sorted set
        count = self.r.zcount(self.job_queue, -float("inf"), float("inf"))
...
def show_results(result):
    try:
        print(
            "%s %s %.2f"
            % (result["status"], fmt_date(result["runtime"]), result["time"])
        )
        print(result["oinfo"])
        print(result["info"])
        if result["status"] == "ok":
