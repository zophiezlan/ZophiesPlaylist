...
class SmarterPlaylistsAdmin(cmd.Cmd):
    ...
    def do_dir(self, line):
        ...
        for user in users:
            ...
            print(print("total programs", total))
    ...
    def do_sprogs(self, line):
        ...
        for user in users:
            ...
            for prog in progs:
                ...
                description = (
                    prog["description"]
                    if prog["description"] != "my description"
                    else ""
                )
        ...
        for pid in line.strip().split():
            ...
            if stats:
                print(
                    ...
                    stats["runs"],
                    "  last run:",
                    fmt_date(stats["last_run"]),
                    "  created:",
                    fmt_date(stats["creation_date"]),
                )
    ...
    def do_jobs(self, line):
        ...
        items = self.my_redis.zrange(
            self.job_queue, 0, count, withscores=True, score_cast_func=int
        )
        for item in items:
            skey, next_time = item
            ...
            if long_form:
                for key, val in self.my_redis.hgetall(skey).items():
                    if key not in self.skips:
                        print("   ", key, val)
                print(print("total jobs", len(items)))
