...
def get_services():
    global pm, scheduler
    if pm is None or scheduler is None:
        import program_manager
        import scheduler as scheduler_mod

        pm = program_manager.ProgramManager(auth, r=my_redis)
        scheduler = scheduler_mod.Scheduler(my_redis, pm)
    return pm, scheduler
...
def inventory():
    start = time.time()
    # Import components lazily to avoid optional deps at startup
    import components
...
def delete():
    ...
    if pid and auth_code:
        ...
        if token:
            user = token["user_id"]
            pm_inst, _ = get_services()
            if pm_inst.delete_program(user, pid):
                results["status"] = "ok"
            else:
                results["status"] = "error"
...
def copy():
    ...
    if pid and auth_code:
        ...
        if token:
            user = token["user_id"]
            pm_inst, _ = get_services()
            pid = pm_inst.copy_program(user, pid)
            if pid:
                results["status"] = "ok"
                results["pid"] = pid
            else:
...
def import_program():
    ...
    if pid and auth_code:
        ...
        if token:
            user = token["user_id"]
            pm_inst, _ = get_services()
            new_pid = pm_inst.import_program(user, pid)
            if new_pid:
                results["status"] = "ok"
...
def directory():
    ...
    if auth_code:
        ...
        if token:
            ...
            pm_inst, sched = get_services()
            total, dir = pm_inst.directory(user, start, count)

            if len(dir) > 0:
                pids = [d["pid"] for d in dir]
                all_ss = sched.get_batch_schedule_status(user, pids)
                for d in dir:
...
def schedule_status():
    ...
    if auth_code:
        ...
        if token:
            user = token["user_id"]
            _, sched = get_services()
            stats = sched.get_run_stats(user, pid)
            rlist = sched.get_recent_results(user, pid)
            results["status"] = "ok"
...
def program():
    ...
    if pid and auth_code:
        ...
        if token:
            user = token["user_id"]
            pm_inst, _ = get_services()
            program = pm_inst.get_program(user, pid)
            if program:
                results["status"] = "ok"
...
def shared():
    ...
    if pid:
        ...
        if "shared" in info:
            ...
            if is_shared:
                owner = info["owner"]
                program = pm_inst.get_program(owner, pid)
                if program:
                    results["status"] = "ok"
                    results["program"] = program
                else:
...
def publish():
    ...
    if not token:
        ...
        share = params["share"]
        pm_inst, _ = get_services()
        pm_inst.publish_program(user, pid, share)
        results["status"] = "ok"
...
def run():
    ...
    pid = params["pid"]
    pm_inst, _ = get_services()
    results = pm_inst.execute_program(auth_code, pid, save_playlist)
    if results["status"] == "ok":
        tracks = []
        results["tracks"] = tracks
        try:
            import pbl  # optional, only for enriching track info
    ...
    if not token:
        ...
        if ok:
            if delta > 0 and total > 0:
                _, sched = get_services()
                if sched.schedule(auth_code, user, pid, when, delta, total):
                    results["status"] = "ok"
                else:
                    results["status"] = "error"
