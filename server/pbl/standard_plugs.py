...
class Split(object):
    ...
    class left_side(object):
        def __init__(self, outer):
            self.outer = outer
            self.name = (
                "first " + str(outer.split_index) + " tracks of " + outer.source.name
            )
...
class Sorter(object):
    ...
    def __init__(self, source, attr, reverse=False, max_size=0):
        self.name = (
            source.name + " sorted by " + attr + ("(reverse)" if reverse else "")
        )
        self.source = source
        self.buffer = []
        self.filling = True
    ...
    def next_track(self):
        while self.filling:
            ...
            if track and (self.max_size == 0 or len(self.buffer) < self.max_size):
                ...
                self.filling = False
                self.buffer.sort(
                    reverse=self.reverse, key=lambda tid: tlib.get_attr(tid, self.attr)
                )
        if len(self.buffer) > 0:
            return self.buffer.pop(0)
        else:
...
class Sample(object):
    ...
    def __init__(self, source, sample_size=10):
        self.name = "Sampling " + str(sample_size) + " tracks from " + source.name
        self.source = source
        self.sample_size = sample_size
        self.buffer = []
        self.filling = True
...
class Case(object):
    ...
    def __init__(self, func, source_map):
        ...
        self.name = "Case of " + ", ".join(
            [n + ":" + s.name for n, s in source_map.items()]
        )
...
class TrackFilter(object):
    ...
    def __init__(self, source, filter, invert=False):
        self.name = (
            source.name + ("inverse" if invert else "") + "filtered by " + filter.name
        )
        self.source = source
        self.filter = filter
...
class ArtistFilter(object):
    ...
    def __init__(self, source, artistNames):
        self.name = (
            source.name + " with songs by " + ", ".join(artistNames) + " removed"
        )
        self.source = source
