...
class EchoNestArtistPlaylist(EchoNestPlaylist):
    ...
    def __init__(self, artist, count):
        ...
        super(EchoNestArtistPlaylist, self).__init__(
            "Artist playlist for " + artist, params
        )
...
def _annotate_tracks_with_echonest_data(tids):
    ...
    if len(otids) > 0:
        ...
        try:
            ...
            response = _get_en().get(
                "song/profile", track_id=uris, bucket=_en_song_buckets
            )
            res = set()
            for song in response["songs"]:
                for track in song["tracks"]:
                    ...
                    if tid in stids:
                        res.add(tid)
                        tlib.annotate_track(
                            tid, "echonest", _flatten_en_song(song, tid)
                        )
            diff = stids - res
            if len(diff) > 0:
                pass
